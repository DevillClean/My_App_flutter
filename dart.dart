...
//Конструктор это Инструкция для получения данных этого класса
...
// Меnоды это функции внутри класса их можно писать любое количество
...
//=================================Наследование
// унаследовать свойства и методы
...
// инклеменция говорит нам о том что мы долждны выписать интерфейсы
...
// ================== Расширение любого существующего класса Exstends
...
/* Выполняемые задачи могут быть 2-х типов Синхронные и Асинхронные 

Синхонные - задачи являютсья Блокирующими 
Следующее дейсвтие будет зхавершенно когда предыдущее было завершенно 


Асинхронные задачи - более интереснее 
Попадают в EventLoop и выполнються в осозначенный EventLoop последовательности 


EventLoop Это бесконечный цикл 
в нем существует 2 очереди 

1 - MicroTask - это приориттетные маленькие задачи
    например освобождение ресурсов
    Используеться редко

2 - Event - это уже более тяжелые события , жесты косания события , стримы
   Асинхронные операции Future 

Future имеет в себе несколько состояний 

  1 не начата или не завершена
  2 завершилась с результатом 
  3 Завершилась с ошибкой

по этому это уже общееный результат 


Future<T>

c Future можно работать двумя способами
Future<void> someAsyncOperation() async {
  ...
}

someAsyncOperation().then{
  (value) => print('on result'),
 для того что бы обработать ошибку нужно пробросить 
 onError: (error, stackTrace) => print('on error'),
};

----- чаще всего работают с await -----
...
print('on result);
  ...
  print ('error');
    ...
    await следует обрабатывать в try catch если мы думаем что может вылезти ошибка

Error - это сама ошибка 
StackTrace - это стек ее вызова
...
// stream - это ассинхронный поток данных

// Стримы бывают двух типов :

// Single subscription - Может иметь максимум одного слушателя
// Stream.value(...);
//  
//  Broadcast
//  может иметь несольких слушателей
//  Stream.value(...)asBroadcastStream();
//
...
//=========================== Ctream Controller - контроллер для управления стримом
// можно прослушивать события и все что есть внутри

// myStreamController<T> myStreamController = StreamController<T>();
// myStreamController.add();
// myStreamController.addError();
// myStreamController.addStream();
// Stream<T> myStream = myStreamCOnttroller.stream;
...
// myStreamController<T> myStreamController = StreamController<T>.broadcast();
// Stream<T> myStreamController = myStreamController.stream;
// 
// ========================================== подписка на стримы
// Stream<T> myStream = const Stream.empty();
// 
// StreamSubscription<T> streamSubscription = myStream.listen(
//  (event) {
//        ...
//  },
//    onError: (error, stackTrace) => print('On error),
...
// );
//
//  Если нам не нужна подписка - не забываем отменять 
...
// например мы можем использовать это в тех случаях когда нужно использовать класс базового класса и 
